...
import { DeviceMotionManager } from "./interaction/device.ts";
import { SVGRenderer } from "./interaction/svg.ts";
...
class CursorRenderer extends BaseGLRenderer {
  ...
  public render(
    ...
    this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, this.buffers.index.buffer);
    ...
    const uniforms: UniformData[] = [
      ...
      {
        name: "uScale",
        value: Math.sqrt(velocity[0] * velocity[0] + velocity[1] * velocity[1]),
        type: "1f",
      },
      {
        name: "uWindow",
    ...
    for (const uniform of uniforms) {
      this.setUniform("cursor", uniform);
    }
...
class OutlineRenderer extends BaseGLRenderer {
  ...
  public render(
    ...
    for (const uniform of uniforms) {
      this.setUniform("outline", uniform);
    }
...
class ParticleSystem {
  ...
  private deviceMotion: DeviceMotionManager;
  ...
  constructor(gl: WebGLContext, size: number) {
    ...
    // デバイスモーションとSVGレンダラーの初期化
    this.deviceMotion = new DeviceMotionManager();
  ...
  private initializeShaders(): void {
    // メインプログラム
    this.mainProgram = createProgram(this.gl, VERTEX_SHADER, FRAGMENT_SHADER);
    if (!this.mainProgram) {
    ...
    const programs = [
      ...
      { name: "velocity", vertex: VERTEX_SHADER, fragment: VELOCITY_SHADER },
      { name: "position", vertex: VERTEX_SHADER, fragment: POSITION_SHADER },
      { name: "advection", vertex: VERTEX_SHADER, fragment: ADVECTION_SHADER },
      {
        name: "divergence",
        vertex: VERTEX_SHADER,
        fragment: DIVERGENCE_SHADER,
      },
      { name: "pressure", vertex: VERTEX_SHADER, fragment: PRESSURE_SHADER },
    ];
    ...
    this.manager.setUniform("velocity", {
      name: "uMVPMatrix",
      value: this.mvpMatrix.getData(),
      type: "mat4",
    });
  }
  ...
  public render(width: number, height: number): void {
    ...
    // デバイスモーションの値を取得
    const acceleration = this.deviceMotion.getAcceleration();

    // パーティクルの更新時にデバイスモーションの値を反映
    this.updateParticles(acceleration);
  ...
  private updateParticles(
    ...
    // 速度の更新（デバイスモーションの影響を加える）
    this.manager.setProgram("velocity");
    this.manager.setUniform("velocity", {
      name: "acceleration",
      value: [acceleration.x, acceleration.y, acceleration.z],
      type: "3f",
    });
    this.manager.run(
      "velocity",
  ...
  public enableDeviceMotion(): void {
    this.deviceMotion.enable();
  }

  public disableDeviceMotion(): void {
